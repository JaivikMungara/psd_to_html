...
//        url: '/ServiceProvider/SPServiceRequest',//        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',        //    });//
...
$(document).on('click', '#NewServiceRequestTabBtn', function () {
            ...
            success: function (result) {
                if (result == "Suceess") {
                    ...
                    window.setTimeout(function () {
                        $("#alertPopup").modal("hide"); window.location.reload();
                    }, 3000);
                }
                    ...
                    var conflictbtn = "#Conflict" + serviceRequestId; $(conflictbtn).removeClass('d-none');
                    window.setTimeout(function () {
                        $("#alertPopup").modal("hide");
                    }, 3000);
                    alert(result);
    ...
    $(".newReqConflictBtn").on('click', function () {
        ...
        $.ajax({
            type: 'GET', url: '/ServiceProvider/ConflictDetails', contentType: 'application/x-www-form-urlencoded; charset=UTF-8', data: data, success: function (result) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    $('#UpcomingServiceTbody').append('<tr class="text-center" data-value=' +
                        ...
                        result[i].startTime + '-' + result[i].endTime + '</td></div>' + '<td class="text-start" data-lable="Customer Details"><div class= "ms-4">' +
                        result[i].customerName + '</div >' + '<div class="d-flex" ><span><img class="me-0" src="/image/layer-15.png" alt=""></span> <span>' +
                        result[i].address + ' </span></div></td>' + '<td data-label="Completed"> <p class= "margin" > <button class="CompleteService d-none">Complete</button></P></td >' + '<td data-label="Action">
                        < p class= "margin" >

                    <button class="upcomingcancel" data-bs-toggle="modal" data-bs-target="#SPdeleteModelServiceRequest">
                        Cancel
                     </button>
                        </P >	
                        </td >
