...
$(document).on('click', '#NewServiceRequestTabBtn', function () {
    ...
    var serviceRequestId = ""; $("#SPServiceRequestTable").click(function (e) {
    ...
    function getMap(zipcode) {
        ...
        marginheight = '0'
        marginwidth = '0'
        src = 'https://maps.google.com/maps?&amp;q=" + encodeURIComponent(zipcode) + "&amp;output=embed' >
            ...
            success: function (result) {
                if (result == "Suceess") {
                    ...
                    window.setTimeout(function () {
                        $("#alertPopup").modal("hide"); window.location.href = url;
                    }, 3000);
                }
    ...
    $(".newReqConflictBtn").on('click', function () {
        ...
        var data = {};
        data.ServiceRequestId = parseInt(id); $.ajax({
            type: 'GET',
            ...
            success: function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    $('#UpcomingServiceTbody').append('<tr class="text-center" data-value=' + result[i].serviceRequestId + ' ><td data-label="Service ID">' +
                        ...
                        < div > <img src="/image/calendar2.png" alt="calender">' + result[i].date + ' </div>' + ' < div >
                    <img src="/image/layer-14.png" alt="clock">' + 
                        result[i].startTime + '-' + result[i].endTime + '</div></td > ' + ' < td class= "text-start" data - lable="Customer Details" >
                    <div class="ms-4">' +
                        result[i].customerName + '</div >' + ' < div class= "d-flex" >
                    <span><img class="me-0" src="/image/layer-15.png" alt=""></span> <span>' +
                        result[i].address + ' </span></div></td > ' + ' < td data - label="Completed" >
                    <p class="margin" >
                        <button class=' + completeclass + '>Complete</button></P></td > ' + ' < td data - label="Action" >
                    <p class="margin"><button data-bs-toggle="modal" data-bs-target="#SPdeleteModelServiceRequest" class="upcomingcancel">Cancel</button></P>	
                        
                        </td >
                        </tr > ')
                }
                if (pagination[0] == 0) {
                    upcomingserviceDatatable(); pagination[0] = 1;
                }
            },
    ...
    $(document).on('click', '#BlockCustomerTabBtn', function () {
        $.ajax({
            ...
            success: function (result) {
                $('#customerGrid').empty();
                for (var i = 0; i < result.length; i++) {
                    var unblock = "d-none"; var block = ""
                    if (result[i].favoriteAndBlocked != null) {
                        var status = result[i].favoriteAndBlocked.isBlocked; if (status == true) {
                            block = "d-none";
unblock = "";

                    }
                    }
                    $('#customerGrid').append('<div  class="col-4 blockCard ">' + '<div>' + '<img class= "cap-icon" src = "/image/cap.png " alt = ".." >' + '</div >' + '<br/>' + '<h3> ' + result[i].user.firstName + '  </h3>' + '<br/>' + '<button id="' + result[i].user.userId + 'B" class="' + block + ' block-cust-btn">Block</button>' + '<button id="' + result[i].user.userId + 'U" class="' + unblock + ' block-cust-btn">Un-Block</button>' + '</div >')
                }
            },
            error: function (error) {
                console.log(error);
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    $('#ServiceHistoryTbody').append(' <tr class= "text-center" data-value=' +
                        result[i].serviceRequestId + ' > <td data-label="Service ID">' + '<p class="margin">' +
                        result[i].serviceRequestId + '</p></td>' + '<td data-label="Service date"> <div><img src="/image/calendar2.png" alt="calender">' +
                        result[i].date + ' </div>' + '<div><img src="/image/layer-14.png" alt="clock">' +
                        result[i].startTime + '-' +
                        result[i].endTime + '</div></td>' + '<td class="text-start" data-lable="Customer Details"><div class= "ms-4">' +
                        result[i].customerName + '</div >' + '<div class="d-flex" ><span><img class="me-0" src="/image/layer-15.png" alt=""></span> <span>' +
                        result[i].address + ' </span>' + '</div></td></tr>');
                }
                if (pagination[1] == 0) {
                    ServiceHistoryDatatable(); pagination[1] = 1;
                }
            },
            error: function (error) {
                console.log(error);
    ...
    $(document).ready(function () {
        ...
        for (var i = 1950; i <= (currentYear); i++) {
            var option = $("<option />"); option.html(i);
            option.val(i); $('#dobyear').append(option);
        }
    });
    ...
    $("#SPSaveChange").on('click', function () {
        ...
        var testnumber = /^[0-9]{10}$/; var testpin = /^[1-9][0-9]{5}$/;
        window.setTimeout(function () {
        ...
        if (data.user.firstName == "") {
            ...
            $("#mSmyDetailsAlert").removeClass("alert-success d-none").addClass("alert-danger").text("Street name is Required."); $("#SPSettingsstreetname").focus();
        }
        else if (!testnumber.test(data.user.mobile)) {
            $("#mSmyDetailsAlert").removeClass("alert-success d-none").addClass("alert-danger").text("Mobile is Invalid."); $("#SPSettingsphonenumber").focus();
        }
        else if (!testpin.test(data.address.postalCode)) {
            $("#mSmyDetailsAlert").removeClass("alert-success d-none").addClass("alert-danger").text("postalcode  is Invalid."); $("#SPSettingspincode").focus();
        }
        else {
            $.ajax({
                type: 'POST',
    ...
    $("#SPChangePassword").on('click', function () {
        ...
        window.setTimeout(function () { $('#mSchangePasswordAlert').addClass('d-none'); }, 5000);
        /*             r'^          (?=.*[A-Z])       
         *             // should contain at least one upper case          (?=.*[a-z])       
         *             // should contain at least one lower case          (?=.*?[0-9])      
         *             // should contain at least one digit          (?=.*?[!@#\$&*~]) 
         *             // should contain at least one Special character          .{8,}             
         *             // Must be at least 8 characters in length        $          */
        var paswordtest = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\da-zA-Z]).{6,14}$/;
        if (data.oldPassword == "" && data.newPassword == "" && data.confirmPassword == "") {
            $("#mSchangePasswordAlert").removeClass("alert-success d-none").addClass("alert-danger").text("All Fields are Required."); $("mSoldpassword").focus();
        }
        else if (data.oldPassword == "") {
            $("#mSchangePasswordAlert").removeClass("alert-success d-none").addClass("alert-danger").text("Old password is Required."); $("mSoldpassword").focus();
        }
        else if (data.newPassword == "") {
            $("#mSchangePasswordAlert").removeClass("alert-success d-none").addClass("alert-danger").text("New password is Required."); $("mSnewpassword").focus();
        }
        else if (!paswordtest.test(data.newPassword)) {
            $("#mSchangePasswordAlert").removeClass("alert-success d-none").addClass("alert-danger").text("please enter strong password."); $("mSnewpassword").focus();
        }
        else if (data.confirmPassword == "") {
            $("#mSchangePasswordAlert").removeClass("alert-success d-none").addClass("alert-danger").text("Confirm password is Required."); $("#mSconfirmpassword").focus();
        }
        else if (data.newPassword != data.confirmPassword) {
            $("#mSchangePasswordAlert").removeClass("alert-success d-none").addClass("alert-danger").text("New Password and Confirm Password must be same."); $("#mSconfirmpassword").focus();
        }
        else {
            $.ajax({
                type: "POST",
    ...
    $.ajax({
        type: 'GET', url: '/ServiceProvider/getRatingData', contentType: 'application/x-www-form-urlencoded; charset=UTF-8', success: function (result) {
            $('#RatingList').empty(); for (var i = 0; i < result.length; i++) {
                var star = ""; for (var j = 1; j < 6; j++) {
                    if (j <= result[i].rating) {
                        star += '<i class="fa fa-star " style="color:#ECB91C; "></i>';
                    }                    else {
    star += '<i class="fa fa-star " style="color:silver;"></i>'
}                                       
                }                star += '<span> &nbsp;' + result[i].remarks + '</span>'                                $('#RatingList').append('<div class="row  rating-row"><div class="row"><div class="col-3">'
    ...
    //paginationvar 
    Newservicerequest = new DataTable("#SPServiceRequestTable", {
        dom: 't<"pagenum d-flex justify-content-between "<"pagenum-left"li><"pagenum-right"p>>',
        responsive: true, pagingType: "full_numbers", language: {
            paginate: {
            ...
            info: "Total Records : _MAX_", lengthMenu: "Show  _MENU_  Entries",
        },
        iDisplayLength: 7, aLengthMenu: [[7, 10, 15, -1], [7, 10, 15, "All"]],
    ...
    function ServiceHistoryDatatable() {
        $("#SPServiceHistoryTable").DataTable({
            ...
            }, iDisplayLength: 7, aLengthMenu: [[7, 10, 15, -1], [7, 10, 15, "All"]], columnDefs: [{ orderable: false, targets: 2 }],
        });
    }
    function upcomingserviceDatatable() {
        $("#SPUpcomingServiceTable").DataTable({
            ...
            language: {
                paginate: {
                    first: "<img src='~/images/pagination-first.png' alt='first'/>",
                    previous: "<img src='~/images/pagination-left.png' alt='previous' />", next: "<img src='~/images/pagination-left.png' alt='next' style='transform: rotate(180deg)' />", last: "<img src='~/images/pagination-first.png' alt='first' style='transform: rotate(180deg) ' />",
                }, info: "Total Records : _MAX_", lengthMenu: "Show  _MENU_  Entries",
            }, iDisplayLength: 7,
