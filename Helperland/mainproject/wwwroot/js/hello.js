function upcoming_service(evt, service) {
    var i, tabcontent, tablinks; tabcontent = document.getElementsByClassName("tab-contant");
...
$('.customerCancel').on('click', function () {
    ...
    alert("Cancel clicked"); document.getElementById("CancelRequestId").value = srId;
});
...
function getAllServiceDetails() {
    ...
    $.ajax({
        type: 'GET', url: '/Customer/DashbordServiceDetails', contentType: 'application/x-www-form-urlencoded; charset=UTF-8', data: data, success: function (result) {
            if (result != null) {
                result.st                showAllServiceRequestDetails(result);
            } else {
...
function showAllServiceRequestDetails(result) {
    ...
    dateTime.innerHTML = result.date.substring(0, 10) + " " + result.startTime + " - " + result.endTime; duration.innerHTML = result.duration + " Hrs";
    extra.innerHTML = "";
    var varStatus = "";
    var dashbtn = "";
    var servicehistorybtn = "";
    switch (result.status) {
        case 1: /*new */            varStatus = "new";
            dashbtn = "";
            servicehistorybtn = "d-none";
            break;
        case 2: /*pending */            varStatus = "pending";
            dashbtn = "";
            servicehistorybtn = "d-none";
            break;
        case 3: /*completed */
    ...
    Status.innerHTML = " Status: <button disabled class=" + varStatus + ">" + varStatus + "</button > "    if (result.cabinet == true) {
...
document.getElementById("confirm_rating").addEventListener("click", function () {
    ...
    data.serviceRequestId = serviceRequestId; $.ajax({
        type: "POST", url: "/Customer/RateServiceProvider", contentType: "application/x-www-form-urlencoded; charset=UTF-8", data: data, success: function (result) {
            if (result.value == "true") {
                $("#myRatingModal").modal("hide"); console.log("submited");
            }
            else {
...
$(document).on('click', '.rateactive', function () {
    ...
    data.ServiceRequestId = parseInt(serviceRequestId);
    $.ajax({
        type: 'GET', url: '/customer/GetRating', contenttype: 'application/x-www-form-urlencoded; charset=utf-8', data: data, success: function (result) {
            if (result == null) {
        ...
        }, error: function () { alert('failed to receive the data'); console.log('failed '); }
    });
});
    ...
    $("#myRatingModal .modal-body").hover(function () {
        $(".ratingfortimearrival").hover(function () {
            $("#ontime1").click(function () {
                ...
                $("#ontime1").css("color", "#ECB91C"); $(".infomsg").text("1");
            });
        ...
        $(".ratingforfriendly").hover(function () {
            $("#friendly1").click(function () {
                ...
                $("#friendly1").css("color", "#ECB91C"); $(".friendlymsg").text("1");
            });
...
$(document).ready(function () {
    var currentYear = (new Date()).getFullYear(); $('#dobyear').append(' <option value="0000">Year</option> ');
...
$(document).ready(function () {
    $.ajax({
        type: 'GET', url: '/Customer/GetCustomerData', contentType: 'application/x-www-form-urlencoded; charset=UTF-8', success: function (result) {
...
$("#CSSaveDetails").on('click', function () {
    ...
    if (data.firstName == "") {
        ...
        $.ajax({
            type: 'POST', url: '/Customer/UpdateCustomerData', contentType: 'application/x-www-form-urlencoded; charset=UTF-8', data: data, success: function (result) {
                if (result.value == "true") {
...
function getAddress() {
    $.ajax({
        type: 'GET', url: '/Customer/GetUserAddress', contentType: 'application/x-www-form-urlencoded; charset=UTF-8', success: function (result) {
            if (result != false) {
                console.log(result);
                $("#alladdress").empty();
                for (var i = 0; i < result.length; i++) {
                    $("#alladdress").append('<tr data-value=' + result[i].addressId + ' >' + '<td><p><strong> Address: </strong>' + result[i].addressLine1 + ", " + result[i].addressLine2 + ', ' + result[i].city + ' - ' + result[i].postalCode + '</p>' + '<p><strong>Phone number: </strong>' + result[i].mobile + '</p></td>' + '<td class="myAddressBtns"><button class="myAddressButton myAddressEditBtn" data-value=' + result[i].addressId + '>' + '<img src="/image/edit.jpg" />  </button > ' + '<button class="myAddressButton myAddressDeleteBtn" data-value=' + result[i].addressId + '>' + ' <img src="/image/deleteiconadd.png" class="deleteiconimg" /> </button> </td > </tr > ');
                }
...
$("#addAddressSubmit").on('click', function () {
    ...
    if (data.addressLine1 == "" && data.addressLine2 == "" && data.postalCode == "" && data.city == "" && data.mobile == "") {
        ...
        $.ajax({
            type: "POST", url: "/Customer/AddNewUserAddress", contentType: "application/x-www-form-urlencoded; charset=UTF-8", data: data, success: function (result) {
                if (result.value == "true") {
                    getAddress();
                    document.getElementById("addressCancelBtn").click(); $("#addNewaddressModel").modal("hide");
                } else {
                    alert("not saved");
                }
            }, error: function (error) { alert(error); }
...
$("#address").on('click', function (e) {
    ...
    if (element != null) {
        if (element.classList.contains("myAddressEditBtn")) {
            addressId = element.getAttribute("data-value"); console.log("editaddress");

            //------------- Get Address Data To Modal -----------------            
            var data = {};
            data.addressId = addressId;
            $.ajax({
                type: 'GET', url: '/Customer/EditAddressModel', contentType: 'application/x-www-form-urlencoded; charset=UTF-8', data: data, success: function (result) {
                    if (result != false) {
            ...
            console.log(addressId); $("#addNewaddressModel").modal("show");
        }
...
$("#updateAddressSubmit").on('click', function () {
    ...
    if (data.addressLine1 == "" && data.addressLine2 == "" && data.postalCode == "" && data.city == "" && data.mobile == "") {
        ...
        $.ajax({
            type: "POST", url: "/Customer/EditUserAddress", contentType: "application/x-www-form-urlencoded; charset=UTF-8", data: data, success: function (result) {
                if (result.value == "true") {
                    getAddress();
                    $("#mSaddAddressAlert").removeClass("alert-danger d-none").addClass("alert-success").text("Address is updated successfully.");
                    window.setTimeout(function () {
                        $("#addNewaddressModel").modal("hide"); document.getElementById("addressCancelBtn").click();
                    }, 500);
                }
                ...
                $("#updateAddressSubmit").addClass("d-none"); $("#addAddressSubmit").removeClass("d-none"); alert(error);
            }
        });
    }
});
$("#MSDeleteAddress").click(function () {
    ...
    data.addressId = addressId;
    $.ajax({
        type: "POST", url: "/Customer/DeleteUserAddress", contentType: 'application/x-www-form-urlencoded; charset=UTF-8', data: data, success: function (result) {
            if (result == true) { getAddress(); } else { alert("fail"); }
        }, error: function (error) { }
    });
});
...
$("#CSChangePassword").on('click', function () {
    ...
    data.confirmPassword = document.getElementById("mSconfirmpassword").value; window.setTimeout(function () {
    ...
    if (data.oldPassword == "" && data.newPassword == "" && data.confirmPassword == "") {
        ...
        $.ajax({
            type: "POST", url: "/Customer/ChangePassword", contentType: "application/x-www-form-urlencoded; charset=UTF-8", data: data, success: function (result) {
                if (result.value == "true") {
...
function getCityFromPostalCode(zip) {
    $.ajax({
        method: "GET", url: "https://api.postalpincode.in/pincode/" + zip, dataType: 'json', cache: false, success: function (result) {
            if (result[0].status == "Error" || result[0].status == "404") {
                ...
                $("#City").val(result[0].PostOffice[0].District); $("#State").val(result[0].PostOffice[0].State).prop("disabled", true);
